...
Original file is located at
    https://colab.research.google.com/github/sitaraiyer/CS131_release/blob/master/spring_2026/hw0_release/hw0.ipynb
...
def complicated_matrix_function(M, a, b):
    ...
    a_dot_b = np.dot(a, b)
    aT = a.T
    M_dot_aT = np.dot(M, aT)
    out = a_dot_b * M_dot_aT
    ### END YOUR CODE
...
def get_eigen_values_and_vectors(M, k):
    ...
    w_abs = np.abs(w)
    sorted_indices = np.argsort(w_abs)[::-1]

    for i in range(k):
        best_i = sorted_indices[i]
        eigenvalues.append(w[best_i])
        eigenvectors.append(v[:, best_i].tolist())
...
> Iterating over the indices of the input image could be possible with a more specific mapping rule (e.g. to fill in empty spaces), but would be more error-prone than the output-iteration approach.

> For instance, if the size of the output image is bigger than the input image. This would mean that the input has fewer pixels than the output, and directly mapping them with a simple row/col * scale_factor rule would leave empty spaces in the output image.

> Furthermore, if the input has more pixels than the output, it could map multiple input pixels to the same output coordinate, which could lead to issues in overwriting data and efficiency. Thus, in both cases, the approach of iterating over the indices of the output image seems like a comparatively better approach to scaling.
...
def rotate2d(point, theta):
    ...
    ### YOUR CODE HERE
    out = np.ndarray(point.shape)
    x_old = point[0]
    y_old = point[1]

    out[0] = (x_old * np.cos(theta)) - (y_old * np.sin(theta))
    out[1] = (x_old * np.sin(theta)) + (y_old * np.cos(theta))
...
> To rotate around a center point (a,b), we can shift the center to the origin by subtracting these coordinates a and b from the relevant x- and y- directions. Then, we can rotate the point, as seen in the function above, before translating the result (rotated point) back to the correct position by adding back the coordinates (a, b) that we previously subtracted.
...
def rotate_image(input_image, theta):
    ...
    for i in range(output_rows):
      for j in range(output_cols):
        rotated_point = rotate2d(np.array([i-center_i,j-center_j]), theta)

        rotated_x = int(rotated_point[0]) + center_i
        rotated_y = int(rotated_point[1]) + center_j

        if((0 <= rotated_x < input_rows) and (0 <= rotated_y < input_cols)):
          output_image[i, j, :] = input_image[rotated_x, rotated_y, :]
